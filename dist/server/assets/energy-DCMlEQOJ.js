...
function getEffectiveDevices(devices) {
...
function generateDaily(devices, days = 30) {
  const source = getEffectiveDevices(devices);
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    return {
      date: date.toLocaleDateString(void 0, { month: "short", day: "numeric" }),
      kwh: Number((avg * variance).toFixed(2)),
      cost: Number((avg * variance * COST_PER_KWH).toFixed(2))
    };
  });
}
function generateMonthly(devices) {
  const source = getEffectiveDevices(devices);
...
function deviceBreakdown(devices) {
  return getEffectiveDevices(devices).filter((d) => d.status).map((d) => ({ name: d.name, value: Number((dailyKwh(d) * 30).toFixed(1)), type: d.type })).sort((a, b) => b.value - a.value);
}
function aiRecommendations(devices) {
  const source = getEffectiveDevices(devices);
  const recs = [];
...
export {
  aiRecommendations as a,
  deviceBreakdown as b,
  generateHourly as c,
  dailyKwh as d,
  generateMonthly as e,
  getEffectiveDevices as f,
  generateDaily as g,
  monthlyCost as h,
  monthlyKwh as i,
  monthlyCO2 as m
